...
public class ValidaImageUtil {
	public static final int WIDTH = 120;// 生成的图片的宽度
	public static final int HEIGHT = 30;// 生成的图片的高度
	public static String RANDOM;//生成的随机数

	public static BufferedImage getNumValidaImage() {
		// 1.在内存中创建一张图片
		...
		RANDOM = drawRandomNum((Graphics2D) g, "n");// 根据客户端传递的createTypeFlag标识生成验证码图片
		// 7.将随机数存在session中
		ActionContext.getContext().getSession().put("random", RANDOM);
		// 8.设置响应头通知浏览器以图片的形式打开
		ServletActionContext.getResponse().setContentType("image/jpeg");// 等同于response.setHeader("Content-Type",
		// "image/jpeg");
		// 9.设置响应头控制浏览器不要缓存
		ServletActionContext.getResponse().setDateHeader("expries", -1);
